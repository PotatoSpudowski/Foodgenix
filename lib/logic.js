...
async function submitLogisticsDetails(foodBatch){
  ...
  async function submitSaleInfo(foodBatch) {
    ...
    if (exists){
      ...
      foodProduct.owner = foodBatch.costumer;
      foodProduct.saleDate = foodBatch.saleDate;
      foodProduct.costumerName = foodBatch.costumerName;
      ...
      event.retailer = foodBatch.retailer;
      event.costumer = foodBatch.costumer;
      event.saleDate = foodBatch.saleDate;
      event.costumerName = foodBatch.costumerName;
      ...
      var participantRegistry = await getParticipantRegistry('org.eyeframe.foodgenix.Costumer');
      await participantRegistry.update(foodBatch.costumer);
  
      foodProduct.owner = foodBatch.costumer;
      participantRegistry = await getParticipantRegistry('org.eyeframe.foodgenix.Costumer');
      await participantRegistry.update(foodBatch.costumer);
